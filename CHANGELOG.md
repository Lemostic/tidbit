...
- **看板视图便签拖动无效**：主窗口此前默认启用 WebView2 原生拖放处理器，压制了网页内 HTML5 拖放，导致看板卡片无法拖动到其他状态列，向分组标签拖动便签同样失效。已禁用原生处理器（应用未使用原生文件拖放），HTML5 拖拽恢复；顺手补齐看板拖放的组件测试与配置回归测试。

## [0.2.13] - 2026-09-25

### 新增

- **自动备份回归**：设置 → 维护 新增“自动备份”开关（默认开启）。每次启动应用自动创建一份加密快照，之后按间隔周期备份；间隔默认 1 小时（可调 0.5–24 小时），自动备份默认保留 20 份（可调 1–100），超出上限时自动清理最旧的自动备份，手动“立即备份”不受影响。
- 自动备份设置持久化到数据目录，调整后立即生效，无需重启。

### 修复

- 自动备份在 0.1.1 引入、早期版本合并时丢失，本次按原加密快照格式恢复，旧备份文件与 0.1.x 的备份设置文件均可继续使用。
